...
2. “逐样本并发请求数”新建默认为 64，可配置 1–128；一个 run 严格按冻结值提交样本链，DSH 宿主对相同 provider 执行全局最多 128 个在飞 stage 请求。
...
- DSH 相同 provider 全局最多 128 个在飞 stage 请求，FIFO 排队；
...
部署后重新截取与 README 对应的网页截图；参数截图必须能看到 500、64、4，运行截图必须区分完整预测时点、评分单元、实际在飞、排队和待提交。
...
7. 恢复自动推进，并观察至少一个完整 origin wave 的提交、完成、失败、实际在飞、排队和待提交变化。

回滚条件包括：历史 replay 失败、事件序号回退/冲突、重复提交、provider 在飞超过 128、页面不能恢复当前 run 或新版本健康检查失败。回滚只切换程序版本，继续保留原数据库和升级前产物；禁止回写或删除事件。
