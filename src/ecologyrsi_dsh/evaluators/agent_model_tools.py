...
def _defaults(config_type):
    values = {'history_steps': 6, 'ridge_alpha': 0.1}
    values.update({name: .5 for name in config_type.__dataclass_fields__
...
class AgentModelTools:
    ...
    def catalog(self):
        return [{
            ...
            'parameters': {key: {'default': value, 'minimum': 1 if key == 'history_steps' else .0001 if key == 'ridge_alpha' else 0,
                           ...
                           for key, value in _defaults(config).items()},
    ...
    def _fit_identity(self, tool_id, config):
        values = {key: .5 if 'residual_scale' in key else value for key, value in config.to_dict().items()}
    ...
    def _origin_input(self, request, config, fit):
        ...
        lag_indices = [self._index.get(request.origin_timestamp - lag) for lag in range(config.history_steps)]
    ...
    def execute(self, requests, tool_id, parameters):
        self._check_control()
        config = _CONFIGS[tool_id].from_mapping({**_defaults(_CONFIGS[tool_id]), **parameters})
