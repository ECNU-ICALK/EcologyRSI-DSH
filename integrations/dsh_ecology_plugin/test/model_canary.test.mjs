...
function request(stage = "generation.search-plan") {
  return { schema_version: CANARY_SCHEMA,
    identity: { provider_id: "provider", model_id: "model", stage, role: STAGES[stage]?.role || "sample-planner",
      preset_id: stage === "generation.reflect" ? "ecology-generation-judge-v8" : "ecology-researcher-v12", output_schema_id: STAGES[stage]?.schema,
...
function fixture(mode = "ok") {
  ...
  const ctx = { sessions: { get: sid => stored.get(sid) },
    ...
    subagents: { async start(provider, value) {
      ...
      const skillName = STAGES[prompt.stage].skillName;
      ...
      if (mode !== "text") { call(3,"skill",{name:skillName},"skill-1"); result(4,"skill-1"); call(5,"structured_output",structured,"result-1"); result(6,"result-1",mode === "rejected"); }
      ...
      const failed = mode === "budget" || mode === "transport" && launches.length === 1;
      return {id:sid,result:Promise.resolve({stopReason:failed?"error":"completed",structured:failed?null:mode === "mismatch"?{success:true}:structured}),dispose:async()=>{}};
    } },
  };
...
test("real canary seam mounts exact roles and requires actual tool/schema captures", async () => {
  for (const stage of ["generation.search-plan","generation.reflect"]) {
    const f=fixture(); const receipt=await f.canary.run(request(stage));
    assert.equal(receipt.passed,true); assert.equal(receipt.scope,"tool_and_schema_transport_only");
...
test("canary bounds and prediction-tool stages fail before any provider request", async()=>{
  ...
  const q=request("sample.plan"); assert.throws(()=>validateCanaryRequest(q),e=>e.code==="model_canary_stage_unsupported");
});
