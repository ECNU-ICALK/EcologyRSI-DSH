...
- An `instruction_profile` direction must set `mutation_direction` to `select`. It changes only Planner reasoning, confidence, reason codes, and protocol reliability for the same origins and numerical tool output. Use it only for observed agent-process failures; it cannot alter sampling, prediction values, numerical forecast metrics when execution is already valid, evidence eligibility, or promotion.
